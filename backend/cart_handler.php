...
class CartHandler {
    ...
    public function __construct() {
        ...
        if (!isset($_SESSION['cart']) && isset($_COOKIE['cart_' . $this->clientId])) {
            $cartData = json_decode($_COOKIE['cart_' . $this->clientId], true);
            $_SESSION['cart'] = is_array($cartData) ? $cartData : [];
        }
    }
    ...
    public function addToCart() {
        ...
        $cartItem = [
            ...
            'event_time' => $_SESSION['booking']['event_time'] ?? '',
    ...
    public function removeExpiredItems() {
        ...
        // Update the cookie after removing expired items
        $this->updateCartCookie();
    }
}
